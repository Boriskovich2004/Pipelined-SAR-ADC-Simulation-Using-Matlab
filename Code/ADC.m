...
%%%%%%%%% TI ADC Work Process %%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 差分理想斜坡信号：Vin_p 从 0 → Vref，Vin_n 从 Vref → 0
Vin_p_tot = linspace(0, Vref, num);
Vin_n_tot = Vref - Vin_p_tot;
...
for i = 1 : n_ch
    ...
    t_sam = t(i,:);
    % Vin_p_chs(i,:) = Vref/2 + Mis_Gain(i).*(Vfs/2)*(2*t_sam/((num-1)*ts)-1) + Mis_OS(i) + sqrt(k*T/C_tot_p1)*randn(1,floor(num/n_ch));
    % Vin_n_chs(i,:) = Vref/2 - Mis_Gain(i).*(Vfs/2)*(2*t_sam/((num-1)*ts)-1)             + sqrt(k*T/C_tot_n1)*randn(1,floor(num/n_ch));

...
Mis_OS = 000.5 * LSB * randn(1,n_ch);       % Inter-channel Offset
Mis_Gain = 1 + 000.2 * LSB * randn(1,n_ch); % Inter-channel Gain Mismatch
Mis_TS = 0.05 * 1/fs * randn(1, n_ch);    % Inter-channel Clock Jitter
...
Cp_p2  = 005e-15; % Stage2 Positive Plate Parasitic Capacitor (F)
Cp_n2  = 005e-15; % Stage2 Negative Plate Parasitic Capacitor (F)

% ResAmp Mismatch & noise
Av         = 80;    % Open-loop Gain (dB)
GBW        = 0.1e9;   % Gain-Bandwidth Product (GHz)
Amp_offset = 0001e-3; % Amplifier Offset (V)
Amp_noise  = 000e-3; % Amplifier RMS Voltage Noise (V)

% SAR Comparator Mismatch & noise
Comp_offset1 = 0005e-3;% Stage1 SAR Comp Offset (V)
Comp_offset2 = 0005e-3;% Stage2 SAR Comp Offset (V)
